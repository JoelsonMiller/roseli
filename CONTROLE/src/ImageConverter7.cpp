...
void locatepoints(const cv_bridge::CvImagePtr img,  ros::ServiceClient imageClient, ros::Publisher pub_points, ros::Publisher pub_vel ){
	...
	// Solução para o erro de detecção de bordas no lado esquerdo do frame
	//--------------------------------------------------------------------
	int y = 0;
	...
	if(imgContours.size()!=0){
		...
		vector<Point> points3(it3.count);
		vector<Point> points4(it4.count);
		...
		for(int index = 0; index<it3.count; index++){
                        buf3.push_back( Vec3b(*it3) );
                        if(buf3[index].val[0] != 0){
                                points3[index]=it3.pos();
                        }
                        it3++;
                }

		for(int index = 0; index<it4.count; index++){
                        buf4.push_back( Vec3b(*it4) );
                        if(buf4[index].val[0] != 0){
                                points4[index]=it4.pos();
                        }
                        it4++;
                }
...
/*		for(int index = 1; index<points0.size(); index++){
		...
		for(int index = 1; index<points1.size(); index++){
			if(points1[index].x - points1[index - 1].x < 10){
				points1.erase(points1.begin()+index);
			}
		}

		for(int index = 1; index<points2.size(); index++){
			if(points2[index].x - points2[index - 1].x < 10){
				points2.erase(points2.begin()+index);
			}
		}

		for(int index = 1; index<points3.size(); index++){
			if(points3[index].x - points3[index - 1].x < 10){
				points3.erase(points3.begin()+index);
			}
		}

		for(int index = 1; index<points4.size(); index++){
...
//================================================================================
		...
		for(int index = 0; index<it1.count; index++){
                        if(points1[index].x !=0){
                                circle(drawingContours, points1[index], 10, Scalar(104,232,244),1,8);
                                //position.push_back(index);
...
//=======================================================================================
		...
		if((p0.size()!=0)){
			...
			points.points_up_up.clear();
			points.points_down_down.clear();
			//cout<<"2"<<endl;
			int i0=0;
			...
			int i3=0;
                        for(vector<Point>::iterator it3 = p3.begin(); it3 != p3.end(); ++it3){
                                geometry_msgs::Point point_aux3;
                                point_aux3.x=(*it3).x;
                                point_aux3.y=(*it3).y;
                                point_aux3.z=0;
                                points.points_up_up.push_back(point_aux3);
                        i3++;
                        }

			int i4=0;
                        for(vector<Point>::iterator it4 = p4.begin(); it4 != p4.end(); ++it4){
                                geometry_msgs::Point point_aux4;
                                point_aux4.x=(*it4).x;
                                point_aux4.y=(*it4).y;
                                point_aux4.z=0;
                                points.points_down_down.push_back(point_aux4);
                        i4++;
                        }
